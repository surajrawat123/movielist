const initalState = {
  ...
  const movies = (state=initalState , action) => {
    switch(action.type){
        case 'releaseDate': return {...action.payload , [action.payload.components[1].item]:action.payload.components[1].items.sort((a,b)=> a.releaseDate - b.releaseDate)};
        case 'rank' : return {...action.payload , [action.payload.components[1].item]:action.payload.components[1].items.sort((a,b)=> a.rank - b.rank)};
